...
$(function () {
    ...
    var fnData = function (obj, pageIndex, recordsNum, pages) {
        ...
        for (var i = 0; i < obj.length; i++) {
            ...
            htmls += [
                ...
                '<div class="flexWrap flexAgCen"  style="width: 33%; text-align:center; "> <div class="flexCon">'
                + op(record.word, record.type) + '</div></div>',
    ...
    getDataList = function () {
        $.ajaxGetData({
            "ajaxUrl": g_requestContextPath + "/setting/getWordList",
            "fnData": fnData,
            "postData": param,
            "headtype": 1
        });
        ...
        submitHandler: function () {
            ...
            var options = {
                success: function (data) {
                    if (data.code == '1000') {
                        window.location.href = g_requestContextPath + "/setting/setWords";
                    } else {
...
function op(word, type) {
    var op = "";
    op += '<a class="a_style" data-word="' + word + '" data-type="' + type + '" onclick="del(this)">删除</a> ';
    return op;
}
...
function del(obj) {
    ...
    var d = dialog({
        ...
        ok: function () {
            $.ajax({
                url: g_requestContextPath + '/setting/delWord',
                data: {
                    'word': word,
                    'type': type
                },
