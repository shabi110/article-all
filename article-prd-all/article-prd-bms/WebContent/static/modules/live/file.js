...
$(function () {
    ...
    getDataList = function () {
        $.ajaxGetData({
            "ajaxUrl": g_requestContextPath + "/live/getLiveRoomFileList",
            "fnData": fnData,
            "postData": {
                'roomId': $('#roomId').val()
            },
        ...
        submitHandler: function () {
            ...
            var options = {
                success: function (data) {
                    if (data.code == '1000') {
                        window.location.href = g_requestContextPath + "/live/toFilePage?roomId=" + $('#roomId').val();
                    } else {
...
//操作
function op(fileId) {
    return '<a class="a_style" data-id="' + fileId + '" onclick="editFileWin(this)">修改</a> ' +
        '<a class="a_style" data-id="' + fileId + '" onclick="deleteFile(this)">删除</a>';
}

//编辑文件
function editFileWin(obj) {
    $.ajax({
        url: g_requestContextPath + '/live/getLiveRoomFileById',
        data: {
            'fileId': $(obj).data("id")
        },
        success: function (data) {
...
function deleteFile(obj) {
    var d = dialog({
        ...
        ok: function () {
            $.ajax({
                url: g_requestContextPath + '/live/deleteFile',
                data: {
                    'fileId': $(obj).data("id")
                },
                success: function (data) {
